...
import { type BuilderInitial, type BuilderStep, fromServerSteps } from "@/components/automations/automation-builder"
...
function expandFromSeeds(rows: SeedRow[]): BuilderStep[] {
  ...
  rows.forEach((r, i) => {
    if (r.parent_index == null) { roots.push(nodes[i]); return }
    ...
    if (template && AUTOMATION_TEMPLATES[template]) {
      ...
      const steps = expandFromSeeds(
        t.steps.map((seed, idx) => ({
          ...
          branch: (seed as { branch?: "yes" | "no" | null }).branch ?? null,
          parent_index: (seed as { parent_index?: number | null }).parent_index ?? null,
        })),
      )
      return {
        name: t.name,
