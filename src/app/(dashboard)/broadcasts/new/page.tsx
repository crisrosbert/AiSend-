...
function NewBroadcastWizard() {
  ...
  // Load draft on mount if ?draft=<id> present
  useEffect(() => {
    ...
    async (opts?: { showToast?: boolean }) => {
      ...
      try {
        ...
        if (isMounted.current) {
          ...
          if (opts?.showToast) {
            toast.success('Draft saved', { description: 'You can resume this broadcast anytime.' });
          }
        }
        return data.id as string;
      } catch (err) {
        if (isMounted.current && opts?.showToast) {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
          ...
          return (
            <div key={step.key} className="flex flex-1 items-center">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => { if (isCompleted) goToStep(index); }}
              ...
              {index < steps.length - 1 && (
                <div className={`mx-3 h-px flex-1 ${index < currentStep ? 'bg-violet-500' : 'bg-slate-800'}`} />
              )}
            </div>
          );
        })}
      ...
      {/* Step Content */}
      <div
        className="relative min-h-[400px] transition-all duration-300 ease-in-out"
...
// Outer page wraps the wizard in Suspense — required by Next.js 14
