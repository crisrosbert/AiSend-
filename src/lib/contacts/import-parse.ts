...
export { XlsxError } from './xlsx-reader';
 ...
 *
 * Uses the dependency-free reader in ./xlsx-reader, which pulls values straight
 * from the sheet XML. That is what keeps long mobile numbers intact: Excel
 * writes a numeric cell as its exact decimal text, so there is no float
 * round-trip and no "9.19877E+11" precision loss to try to undo.
 */
export async function parseWorkbook(buffer: ArrayBuffer, sheetName?: string): Promise<Sheet> {
  ...
  const worksheet = workbook.readSheet(active);

  const sheet = toSheet(worksheet.rows.map((row) => row.map((cell) => String(cell ?? ''))));
  return { ...sheet, sheetNames, activeSheet: active };
}

const SPREADSHEET_EXTENSIONS = /\.(xlsx|xlsm)$/i;
/** Formats we can detect but not read — the user gets a "re-save as .xlsx" hint. */
const LEGACY_SPREADSHEET_EXTENSIONS = /\.(xlsb|xls|ods|numbers)$/i;

export function isSpreadsheetFile(fileName: string): boolean {
  return SPREADSHEET_EXTENSIONS.test(fileName.trim());
}

export function isLegacySpreadsheetFile(fileName: string): boolean {
  return LEGACY_SPREADSHEET_EXTENSIONS.test(fileName.trim());
}
...
export async function readContactFile(file: File, sheetName?: string): Promise<Sheet> {
  if (isLegacySpreadsheetFile(file.name)) {
    throw new XlsxError(
      'That is an older spreadsheet format. Open it in Excel and use ' +
  ...
  // A mislabelled .csv that is really a workbook is common when people rename
  // files by hand — sniff the ZIP magic rather than trusting the extension.
  const buffer = await file.arrayBuffer();
  if (looksLikeXlsx(buffer)) return parseWorkbook(buffer, sheetName);
