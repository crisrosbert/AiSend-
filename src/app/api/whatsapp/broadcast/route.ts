...
/**
 * The reply pipeline runs after the response, so this is the budget for
 * that work rather than for Meta's wait. Sixty seconds is the Hobby
 * ceiling; raise it if the plan allows and journeys grow longer.
 */
...
export async function POST(request: Request) {
  ...
  //
  // This used to `await processWebhook(body)`, which meant holding the
  // webhook open while the AI composed a reply — one to five seconds,
  // sometimes more. Meta retries a webhook it considers slow, and each
  // retry re-ran the whole pipeline: two replies to the customer, two
  // model calls billed to us. More traffic made it worse, not better,
  // and sustained slowness gets a webhook disabled outright.
  //
  // after() runs the same work once the 200 has gone out, so Meta sees
  // a response in milliseconds. Nothing about the processing changes —
  // only when it happens relative to the reply.
  after(async () => {
    try {
      await processWebhook(body)
    } catch (error) {
      // Nothing is listening any more, so this log is the only trace a
      // failure leaves. Keep it loud.
...
async function processMessage(
  ...
  if (msgError) {
    ...
    // exactly one thing: Meta has sent us this message before. Migration
    // 023 added the constraint precisely so a retry lands here instead
    // of producing a second reply to the customer and a second model
    // call on our bill.
    //
    // Returning early is the whole mechanism. The database settles the
    // race, so two webhooks arriving at once cannot both proceed — one
    // inserts, the other violates, and only the first goes on to reply.
